...
function AddEditData(id) {
    ...
    $('#editPanel').load(url, vm, function (response, status, xhr) {
        if (status === "success") {
            $('#PeriodIntervalLabelOriginalVal').val($('#PeriodIntervalLabel').text());
...
function EnableDisablePeriodInterval(freq) {
    ...
    var periodIntervalLabelText = $('#PeriodIntervalLabelOriginalVal').val();

    // set period interval label user friendly according selection
    if (freq === "DOW") {
        ;
        periodIntervalLabelText += " [Enter value 1 for Sunday ... 6 for Saturday]";
    }
    else if (freq === "DOM") {
        periodIntervalLabelText += " [Enter value between 1 and 31]";
    }

    $('#PeriodIntervalLabel').text(periodIntervalLabelText);
}